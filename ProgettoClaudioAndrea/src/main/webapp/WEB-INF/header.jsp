				...
				<div class="col-lg-2 col-md-2 col-sm-6 right-bar-ord">
					<div class="right-bar">
						...
						<div class="sinlge-bar">
							<a href="/actions/VisualizzaAccount" class="single-icon"><i
						...
						<div class="sinlge-bar shopping">
							...
							%>
							<a href="/actions/privato/carrello/Cart" class="single-icon"><i
								class="fa fa-shopping-cart"></i> <span class="total-count"><%=totalCount%></span>
							</a>
							<%
							} else {
							%>
							<a href="/actions/privato/carrello/Cart" class="single-icon"><i
								class="fa fa-shopping-cart"></i> <span class="total-count">0</span>
							</a>
							<%
							}
							%>
							...
							<div class="shopping-item">
								<div class="dropdown-cart-header">
									<span><%=totalCount%> Item/s</span> <a
										href="/actions/privato/carrello/Cart">View Cart</a>
								</div>
								<ul class="shopping-list">
									...
									%>
									<li><a
										href="/privato/cliente/carrello/RemoveArticle?id=<%=item.getIdArticolo()%>"
										class="remove" title="Remove this item"><i
											class="fa fa-remove"></i></a> <a class="cart-img"
										href="/ProductDetails?id=<%=item.getIdArticolo()%>"><img
											src="https://via.placeholder.com/70x70" alt="#"></a>
										<h4>
											<a href="/ProductDetails?id=<%=item.getIdArticolo()%>"><%=item.getNomeProdotto()%></a>
										</h4>
										<p class="quantity"><%=item.getQuantita()%>x - <span
												class="amount">€<%=item.getPrezzoAcquisto()%></span>
										</p></li>
									<%
									}
									}
								...
								%>
								<div class="bottom">
									<a href="/privato/cliente/ordine/RichiediCheckOut"
...
<%		
	...
	out.write("class=\"active\"");%>><a
												href="/actions/privato/articolo/Search">SHOPPING</a></li>
											<%
											for (Categoria categoria : categoriaDAO.getAll()) {
											%>
											<li
												<%if (ref != null && category_url != null && category_url == categoria.getIdCategoria())
	out.write("class=\"active\"");%>><a
												href="/Search?category=<%= categoria.getIdCategoria() %>&ref=nav"><%= categoria.getNome() %></a></li>
											<%	
			                                    	}
			                                    %>
										</ul>
